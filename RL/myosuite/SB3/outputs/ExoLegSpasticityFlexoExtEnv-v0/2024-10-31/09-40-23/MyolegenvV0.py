...
class Myoleg_env_v0(BaseV0,env_base.MujocoEnv):
    ...
    def get_inter_force_penalty(self, torque, force_threshold):
        penalty_coefficient = 0.03744665341942489
        inter_force_penalty = np.absolute(torque)
        excess_force = np.abs(inter_force_penalty) - force_threshold
    
        # Apply exponential penalty only if the excess force is positive
        if excess_force > 0:
            inter_force_penalty = -1+np.exp(penalty_coefficient * excess_force)
        else:
    ...
    def get_reward_dict(self, obs_dict):
        ...
        #Interaction penalty
        inter_force_penalty = self.get_inter_force_penalty(torque=inter_force_leg, force_threshold=force_threshold)
        #Action penalty 
