...
class Myoleg_env_v0(BaseV0,env_base.MujocoEnv):
    ...
    DEFAULT_OBS_KEYS = ['time', 'qpos', 'qvel', 'qacc', 'act', 'qfrc_constraint', 'pose_err', 'inter_force']
    ...
    def get_obs_dict(self, sim):
        ...
        obs_dict['inter_force'] = np.absolute([self.get_inter_force(pos=sim.data.qpos[6], torque=sim.data.qfrc_constraint[6])])
        return obs_dict
    ...
    def get_inter_force(self, pos, torque):
        ...
        inter_force_penalty = torque_ext
    ...
    def get_pose_reward(self, pose_err, vel):
        ...
        lineal_reward = pose_err*(1/(1.57-self.target_qpos[0]))
        pose_reward = pose_reward - lineal_reward
        if (pose_err < 0.05) and (vel< 0.1):
            pose_reward = pose_reward*5
        return pose_reward
    
    def get_inter_force_penalty(self, torque):
        inter_force_penalty = torque
        return inter_force_penalty
    
    def get_vel_penalty(self, knee_vel):
        if knee_vel > 4:
            velocity_penalty = np.absolute(knee_vel)
        else:
            velocity_penalty = 0
    ...
    def get_reward_dict(self, obs_dict):
        ...
        #Velocity penalty
        velocity_penalty = self.get_vel_penalty(knee_vel=knee_vel)
        #Interaction penalty

        inter_force_penalty = self.get_inter_force_penalty(torque=inter_force_leg)

        exo_torque_generator = (np.absolute(obs_dict_squeeze['act'][0]))
        done_high = ((actual_pos > self.target_jnt_range[0][1]) & (inter_force_leg > 50)).astype(float)
        done_low = ((actual_pos < self.target_jnt_range[0][0]) & (inter_force_leg > 50)).astype(float)
        #print("Act_penalty:", -1*(exo_torque_generator)*0.05, "Pos_score", pose_reward*0.1, "Interaction_penalty", -1*inter_force_penalty*0.001)
        ...
        rwd_dict = collections.OrderedDict((
            ...
            ('act_reg', -1*(exo_torque_generator)),
