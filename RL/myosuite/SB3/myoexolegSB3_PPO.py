...
def make_env(env_id: str, num_env = 0):
    """
...
def main(cfg):  # noqa: F821
    ...
    env = Monitor(gym.make(env_name))  # Env Monitor for eval callbacks
    # Log Path
    log_path = f'.'
...
# --------------------------------------------------------------------
    #Observation Space
    #env = VecNormalize(envs, norm_obs=True, norm_reward=False)
...
# --------------------------------------------------------------------
    ...
    model = PPO(
                ...
                n_steps=cfg.collector.batch_size,
                batch_size=cfg.hiperparameters.mini_batch_size,
                n_epochs=cfg.hiperparameters.ppo_epochs,
                gamma=cfg.hiperparameters.gamma,
                policy_kwargs=policy_kwargs,
                tensorboard_log=tensorlog_path,
                gae_lambda=cfg.hiperparameters.gae_lambda,
                clip_range=cfg.hiperparameters.clip_range,
                #clip_range_vf=cfg.hiperparameters.clip_range_vf,
                ent_coef=cfg.hiperparameters.entropy_coef,
                vf_coef=cfg.hiperparameters.critic_coef, 
                use_sde=cfg.hiperparameters.use_sde,
                verbose=0,
                device="cuda")
