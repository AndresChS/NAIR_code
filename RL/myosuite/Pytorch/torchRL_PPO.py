...
def make_env(env_name="", device="cpu", target_qpos=None):
    if target_qpos is not None:
        print(target_qpos)
        if isinstance(target_qpos, torch.Tensor):
            target_qpos = target_qpos.item()
        env = GymWrapper(gym.make(env_name, target_qpos=target_qpos), device=device)
    else: 
...
def make_ppo_models_state(proof_environment):
    ...
    inputs = proof_environment.observation_spec["observation"]
    # Define policy output distribution class
    num_outputs = proof_environment.action_spec.shape[-1]
    #print(proof_environment.action_spec.shape[-1])
    distribution_class = TanhNormal
    distribution_kwargs = {
        "min": -1, #proof_environment.action_spec.space.low,
        "max": 1, #proof_environment.action_spec.space.high,
        "tanh_loc": False,
    }
...
def main(cfg: "DictConfig"):  # noqa: F821
    ...
    for i, data in enumerate(collector):
        ...
        if i == 100000:
            print("----",data['action'],"----")
            action_data = np.array(data['action'])
            np.savetxt('datos.txt', action_data)
        #create_env_fn.mj_render()
        log_info = {}
        ...
        if len(episode_rewards) > 0:
            ...
            print(episode_rewards.mean().item())
        ...
        with torch.no_grad(), set_exploration_type(ExplorationType.MODE):
            if ((i - 1) * frames_in_batch) // cfg_logger_test_interval < (
                ...
                #print("----",data['episode_reward'],"----")
                np.savetxt('observation.txt', np.array(data['observation']))
        ...
        #Save actor if has the best score 

        
        avg_reward = episode_rewards.mean().item()
        if avg_reward > best_score:
            best_score = avg_reward
            save_weights_to_txt(actor, filename="trained_weights.txt")
    ...
    with torch.no_grad(), set_exploration_type(ExplorationType.MODE):
        ...
        while not done:
            actor.eval()
            #test_rewards = eval_model(
            #        actor, test_env, num_episodes=1
            #    )
            loc, scale, action, prob = actor(state)
...
# Wandb Server ID
#034bc7a6fb926323feaee6dd6f1c506ee0d64d98
