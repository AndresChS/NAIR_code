...
def make_env(env_id: str, num_env = 0, seed: int = 0, delayed_sensors=True):
    ...
    """ 
    def _init():
        env = gym.make(env_id,use_delayed_sensors=delayed_sensors, num_envs=num_env, asynchronous=False)
...
def main(cfg):  # noqa: F821
    ...
    envs = DummyVecEnv([make_env(env_name,i, 0, True) for i in range(num_cpu)])
