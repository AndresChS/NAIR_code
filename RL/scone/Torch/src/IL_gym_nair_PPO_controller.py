...
# --------------------------------------------------------------------
trainning_path = "/home/achs/Documents/achs/code/NAIR_code/RL/scone/Torch/outputs/IL/nair_walk_h0918-v1/2025-07-09/13-14-57"
...
def scone_step(model, muscles_actions, motor_torque, use_neural_delays=True, step=0):
	...
	mus_in = muscles_actions
	#mus_in = np.concatenate((muscle_activations,motor_torque))
	#print(mus_in)
	print("pre: ",model.actuator_input_array()[0])
	model.set_actuator_inputs(mus_in)
	print("post: ",model.actuator_input_array()[0])
...
for step in range(timesteps):
	...
	actions, (state, reward, terminated, info) = TorchRL_controller(state, env_eval, step*timestep, timesteps, device=device)
	print(actions[0])
	motor_torque = np.zeros(1)
