...
def measure_performance(model_file):
	...
	start_time = time.perf_counter()
	while duration < 2.0:
		run_simulation(model, False, True, random_seed, max_time=2, min_com_height=-10)
...
if sconepy.is_supported("ModelOpenSim3"):
	model = sconepy.load_model("test/Gait2D - GeyerHerr2010 - OpenSim3.scone")
	measure_performance("test/Gait2D - GeyerHerr2010 - OpenSim3.scone")
